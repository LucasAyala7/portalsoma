...
/**
 * Endpoint markdown raw · ingest-friendly pra LLM crawlers.
 * Acessado via middleware quando URL termina em .md.
 *
 * Suporta:
 *  - /mensagem-de-aniversario/.md             → hub do nicho
 *  - /mensagem-de-aniversario/para-mae/.md    → cluster
 *  - /mensagem-de-aniversario/para-mae/feliz/.md → single
 */
export async function GET(req: Request) {
  const { searchParams } = new URL(req.url);
  const rawPath = (searchParams.get("path") ?? "").replace(/\.md$/, "").replace(/\/$/, "");
  ...
  try {
    // /<nicho>/                    → nicho hub
    // /<nicho>/<cluster>/          → cluster
    // /<nicho>/<cluster>/<slug>/   → single mensagem
